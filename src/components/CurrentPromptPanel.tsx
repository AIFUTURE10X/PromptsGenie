...
interface Props {
  prompt: string;
  source?: "edge" | "gemini-mm" | "gemini-text" | "subject" | "scene" | "style" | "subject+scene" | "subject+style" | "scene+style" | "subject+scene+style";
  onCopy: () => void;
  onEdit: () => void;
  onClear: () => void;
  onRegenerate: () => void;
  ...
  return (
    <div className="bg-panel dark:bg-dark-panel rounded-xl border-2 border-border dark:border-dark-border h-full flex flex-col">
      ...
      <div className="p-4 overflow-auto">
        {prompt ? (
          <p className="whitespace-pre-wrap text-xs text-text-primary dark:text-dark-text-primary">{prompt}</p>
        ) : (
          <p className="text-xs text-text-secondary dark:text-dark-text-secondary">No prompt yet. Generate one from the editor or drop images.</p>
        )}
      </div>
    </div>
  );
