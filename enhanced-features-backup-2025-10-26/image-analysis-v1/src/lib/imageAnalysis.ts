...
function getAnalysisPrompt(analysisType: string, speedMode: string): string {
  ...
  switch (analysisType) {
    ...
    case 'style':
      return speedMode === 'Quality'
        ? `${basePrompt} Focus on the artistic style, visual treatment, and overall aesthetic approach, but avoid technical photography terms.`
        : `${basePrompt} Describe the visual style and aesthetic.`;
...
export async function analyzeImagesFactual(
  ...
  try {
    ...
    return {
      analysis: `Failed to analyze ${analysisType}: ${error?.message || 'Unknown error'}`,
      confidence: 'LOW',
      filteredTokens: [],
      metadata: {
        model,
